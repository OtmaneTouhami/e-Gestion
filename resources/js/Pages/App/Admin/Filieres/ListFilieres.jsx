...
function ListFilieres({ filieres, counts }) {
    ...
    const destroy = () => {
        router.delete(route("filieres.destroy", del.id));
        hideDelete();
    };

    const submitHandler = (e) => {
        e.preventDefault();
        submit("get", route("filieres.index"), {
            preserveState: true,
        });
    };

    const getFiliere = (id) =>
        filieres.data.find((filiere) => Number(filiere.id) === Number(id));

    const getLibelle = (id) => getFiliere(id).libelle;

    const clearSearch = () => {
        reset();
        router.get(route("filieres.index"));
    };
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div className="flex items-center justify-end h-24 rounded  dark:bg-gray-800">
                    <div className="text-2xl text-gray-400 dark:text-gray-500 ml-4">
                        ...
                        >
                            Ajouter une filière
                            <MdLibraryAdd className="w-5 h-5 ml-2 -mr-1" />
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="grid gap-4 mb-4">
                {filieres.data.length > 0 ? (
                    ...
                    <div className="flex items-center col-span-3 justify-center h-96 rounded  dark:bg-gray-800">
                        <div className="text-3xl  text-gray-400 dark:text-gray-500 ml-4">
                            <div className="text-xxl text-black font-bold dark:text-white">
                                Pas de filières pour le momment!
                            </div>
                        </div>
                    </div>
                )}
            </div>
            {counts.nbFilieres > 10 && (
                <div className="flex items-center justify-center h-16 mb-4 rounded  dark:bg-gray-800">
                    <Link
                        href={filieres.prev_page_url}
                        disabled={filieres.prev_page_url === null}
                        as="button"
                    ...
                    <Link
                        href={filieres.next_page_url}
                        as="button"
                        disabled={filieres.next_page_url === null}
                        className="inline-flex text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-green-400 font-medium rounded-lg text-sm px-4 py-2 mr-3 text-center inline-flex items-center dark:bg-green-500 dark:hover:bg-green-600 dark:focus:ring-green-700"
                    >
                        Next
                        <svg
            ...
            {edit.inEdit && (
                <Edit end={hideEdit} filiere={getFiliere(edit.id)} />
            )}
            {del.inDelete && (
                <Delete
